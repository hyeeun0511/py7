# # for 변수 in 범위:
# # pass - 빈공백 : for,if 한줄이라도 코드가 없으면 에러가 나기에 pass
# # beak - 반복문을 종료
# # continue - 1회 반복문을 제외시켜줌.  # 무조건 if가 따라옴
# for i in range(10):
#     pass  # 아무것도 일어나지 않음 - 빈공백 
#     print("프로그램 종료")  # for 문 밖에 있을 시 - 1번실행  /  for 문 안에 있을 시 - 10번실행
#     break  # 이 시점에서 반복문을 종료  

# for i in range(10):
#     if i==3:
#         continue  # 1회만 제외
#     print(i)
    
# for i in range(10):
#     if i%2 ==0:
#         continue  # 1회만 제외
#     print(i)  # 0,2,4,6,8(continue. 즉 패스) / 1,3,5,7,9(출력)
    
# for i in range(10):
#     if i%2 ==0:
#         print("프로그램 종료")  
#         continue  # 1회만 제외


# for i in range(2,10):        # 8번 실행
#     for j in range(1,10):    # 9번 실행
#         print("숫자 : ",j)    # 1~72
#         #print("-"*50)       # 72번 실행
#     print("-"*50)      

# for i in range(2,10):          # 반복
#     print(i)
#     print("-"*50)              # 8번 실행

# for i in range(2,10):          # 반복
#     print(i)
#     for j in range(1,10):
#         pass
# print("-"*50)              # 1번 실행
    
# for i in range(2,10):          # 반복
#     print(i)
#     for j in range(1,10):
#         pass
#         print("-"*50)              # 72번 실행 (9번*8번)

# for i in range(2,10):          # 8
#     for j in range(1,10):      # 9
#         print(i)               # 72번 실행 (9번*8번)
        
# for i in range(2,10):          # 8
#     for j in range(1,10):      # 9
#         print(j)               # 1~9 8번 실행
        
# for i in range(2,10):          # 8 -2,3,4,5,6,7,8,9
#     print(i)
    
# for i in range(8):          # 8 -0,1,2,3,4,5,6,7
#     print(i)
    
    
# ## 5,21까지 출력하시오.
# for i in range(5,22):
#     print(i)

# ## 1,10까지 출력하시오.
# for i in range(1,11):
#     print(i)

# ## 0,9까지 출력하시오. 홀수만 출력하시오.
# # 풀이 1
# for i in range(10):
#     if i%2 == 0:
#         continue
#     print(i)
    
# # 풀이 2    
# for i in range(10):    
#     if i%2 == 1:
#         print(i)
        
# ## 구구단을 출력하시오.
# # for i in range(2,10):
# #     if i%2 == 0:
# #         continue
# #     print("안녕",+i)
# #     for j in range(1,10):
# #         print("{} X {} = {}".format(i,j,i*j))
# #     print()
# ###### 중요 # 과제 ######
# # for i in range(2,10):
# #     if i%2 == 0:
# #         continue
# #     print("[ {} 단 ]".format(i))
# #     for j in range(1,10):
# #         print("{} X {} = {}".format(i,j,i*j))
# #     print()
    
    
# names = ['홍길동','유관순','이순신','김구','강감찬']
# # for 변수 in 범위: -> range,list,문자열,딕셔너리,튜플

# for name in names:
#     print(name)       # 1.홍길동 2.유관순 ...
    
# for name in enumerate(names): # index번호, 값을 리턴
#     print(name[0],name[1])
    
# for i,name in enumerate(names): # index번호, 값을 리턴
#     print("{}.{}".format(i,name))    # 0번부터 나오게 됨

# for i,name in enumerate(names): # index번호, 값을 리턴
#     print("{}.{}".format(i+1,name))    # 1번부터 나오게 됨
    
# n_list = [               # 2차원 list   # 2차원배열은 for문을 2번 써야함
#     [1,'홍길동'],
#     [2,'유관순'],
#     [3,'이순신']
# ]
# for ns in n_list:  #[[1,'홍길동'],[2,'유관순'],[3,'이순신']]
#     for n in ns:
#         print(n)  
#     print()
    

a_list = []
# for문을 사용해서 0을 10개 들어가는 리스트를 만드시오.
# append 추가
for i in range(10):
    a_list.append(0)
print(a_list)

# 리스트 타입 변환
a_list = list('0'*10)   # 0을 문자열로 넣어줘야함. 숫자 0으로 작성시 수치로 에러남
print(a_list)

# 리스트 내포
a_list = ['0' for _ in range(10)]  
print(a_list)
a_list = [0 for _ in range(10)]    # 숫자 0으로 작성해도 가능
print(a_list)

a_list = [i*i for i in range(1,10)]
print(a_list)